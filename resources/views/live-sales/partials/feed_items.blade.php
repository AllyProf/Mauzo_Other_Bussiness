...
<div class="media border-bottom py-3 px-1 live-feed-item" data-sale-id="{{ $sale->id }}">
  <div class="mr-3 text-center" style="min-width: 52px;">
    ...
    <span class="badge badge-{{ $statusClass }}">{{ strtoupper($sale->payment_status) }}</span>
  </div>
  <div class="media-body">
    <div class="d-flex justify-content-between align-items-start">
      <div>
        <strong>{{ $sale->reference_no }}</strong>
        <span class="text-muted small"> · {{ $sale->user?->name ?? 'Staff' }}</span>
    ...
    <small class="text-muted">
      {{ $sale->usesServices() ? 'Service' : 'Store' }}
...
<div class="text-center text-muted py-5">
  ...
  <p class="mb-0">No sales in this period yet.</p>
</div>
@endforelse
