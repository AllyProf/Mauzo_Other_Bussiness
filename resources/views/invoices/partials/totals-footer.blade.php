...
@endphp
<tr>
  <th colspan="5" class="text-right">{{ $vatBreakdown ? 'Subtotal (excl. VAT)' : 'Subtotal' }}</th>
  <th class="text-right">{{ money($vatBreakdown['subtotal_excl'] ?? $sale->total_amount) }}</th>
</tr>
@if($vatBreakdown)
<tr>
  <th colspan="5" class="text-right">VAT ({{ rtrim(rtrim(number_format($vatBreakdown['rate'], 2), '0'), '.') }}%)</th>
